...
function Stats() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const apiUrl = `${process.env.REACT_APP_BACKEND_URL}/api/main/stats/`;
  ...
  const handleButtonClick = async (e) => {
    ...
    try {
      ...
      let apiUrl = `${process.env.REACT_APP_BACKEND_URL}/api/main/stats/${type}/`;
      setOver3Month(true);

      if (type === "weekly") {
        setOver3Month(false);
        console.log(over3Month);
        apiUrl = `${process.env.REACT_APP_BACKEND_URL}/api/main/stats/`;
      }
